...
It has some extra cases where if it is near the top or bottom of the page and those sections are too small for them to be the one that gets selected, they are forcibly selected if visible, with some tolerance. 
This was especially important for the first section, as it is often too small to be the one selected otherwise.
