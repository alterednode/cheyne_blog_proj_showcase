...
As this is a site which primarily exists for me to write about things that I've programmed, I wanted to have nice code blocks with syntax highlighting.

I'm already using **rehype-pretty-code** to handle syntax highlighting, but I needed to also provide some custom CSS to avoid it looking like this:
...
On top of all the stuff needed to get the site up and running, looking and nice, and doing what I need, there are a few extra features I've added.

### RSS Feed

I discovered RSS feeds while researching how other blogs and portfolio sites were built, ([Josh W Comeau](https://www.joshwcomeau.com/blog/how-i-built-my-blog-v2/) was an inspiration for several parts of this site, his [book](https://www.joshwcomeau.com/effective-portfolio/) on developer portfolios was also a great resource) and thought it would be a nice addition to let people subscribe to my posts.

I used the [feed](https://www.npmjs.com/package/feed) package to generate an RSS feed at build time with the post metadata which was already being extracted from the MDX frontmatter.
...
To add that to my posts, I made sure every heading automatically creates its own link by converting the generated html headers to anchor links with a custom component. Clicking a heading smoothly scrolls back to that spot, and I spent some extra time on some QOL stuff like gracefully replacing the current url and ensuring stuff like opening links in a new tab work as expected.

With that in place it was pretty straightforward to generate a table of contents that scrolls you to that location.
I end up seperately parsing the MDX content at build time with some simple regex to extract the headings for the table of contents, and made a custom slug generator to ensure the links match up between the two locations.
...
I still don't like how if you use the TOC to navigate to the first section it highlights the second one if the first isn't long enough to put the second below the midpoint, but it's someting im willing to live with for now.

One extra thing I added was if the footer is visible, the TOC highlights the last entry regardless of scroll position.

## What's Next?

The basics of this site are pretty much where I want them as of the time of writing for the purpose of being able to create these posts, but I plan on adding more.

To start, I'm planning on adding a page with a few browser-based tools I've made, and a space for me to put some of the games I've made that can be played with HTML5.
